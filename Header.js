...
import useCart from '../../hooks/useCart'; // Импортируем хук для доступа к корзине
...
const Header = () => {
  ...
  // Получаем актуальное количество товаров из контекста корзины
  const { totalItems } = useCart();
  
  // Обновляем значение поля поиска только при первоначальном рендеринге
  // или при изменении маршрута (чтобы избежать циклических обновлений)
  useEffect(() => {
    if (location.pathname === '/catalog' && !location.search.includes('updating=true')) {
      ...
      if (searchParam) {
        ...
        // Если параметр search отсутствует, сбрасываем поле поиска
        setSearchQuery('');
      }
    }
  ...
  const handleSearchSubmit = (e) => {
    ...
    if (searchQuery.trim()) {
      ...
      // Добавляем специальный параметр, чтобы предотвратить циклические обновления
      searchParams.set('updating', 'true');
    }
    
    // Перенаправляем на страницу каталога с параметрами поиска
    navigate({
      pathname: '/catalog',
      search: searchParams.toString()
    });
    
    // Через небольшую задержку удаляем параметр updating из URL
    setTimeout(() => {
