...
const OrderConfirmationPage = () => {
  ...
  // Если пользователь перешел на страницу подтверждения, но в корзине есть товары,
  // значит заказ не был оформлен должным образом
  useEffect(() => {
    if (items.length > 0) {
      navigate('/cart');
    }
  ...
  // Если ID заказа не передан, перенаправляем на главную
  useEffect(() => {
    if (!id) {
      navigate('/');
    }
  ...
  // Расчётная дата доставки (через 3-5 дней)
