...
const CatalogPage = () => {
  ...
  // Начальные фильтры из URL-параметров
  ...
  // Используем хук для работы с товарами
  const {
    products,
    loading,
    error,
  ...
  // Отслеживаем изменения в URL и обновляем фильтры
  useEffect(() => {
    if (skipFilterUpdate) {
  ...
  useEffect(() => {
    ...
    // Добавляем только установленные фильтры в URL
  ...
  // Генерируем заголовок на основе выбранной категории или поискового запроса
  const getPageTitle = () => {
    if (filters.search) {
